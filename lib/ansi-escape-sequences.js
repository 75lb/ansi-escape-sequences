 ...
 * '\u001b[32;1mwhat?\u001b[0m'
 */
ansi.format = function (str, styleArray, options) {
  options = options || {}
  var re = /\[([\w\s-]+)\]{([^]*?)}/
  var matches
  if (!str) return ''
  ...
  if (styleArray && styleArray.length) {
    if (options.eachWord) {
      return wrap.getWords(str)
        .map(function (word) {
          if (newLine.test(word)) {
            return word
          } else {
            return ansi.styles(styleArray) + word + ansi.style.reset
          }
        })
        .reduce(function (prev, curr) {
          if (newLine.test(curr) || newLine.test(prev)) {
            return prev + curr
          } else {
            return prev + ' ' + curr
          }
      ...
      return ansi.styles(styleArray) + str + ansi.style.reset
    }
  } else {
    return str
  }
